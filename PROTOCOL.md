...
для записи без текста с одним файлом и является best-effort, а не DRM. Только
успешный полный GET без Range и без `inline=1` удаляет запись. HEAD, 304, 412,
ошибка записи/flush и отмена не расходуют файл. Архив также удаляется только после
успешной передачи. Encrypted client после чтения всех чанков подтверждает получение
через `POST /api/rooms/{room}/files/{file}/consume`; предпросмотр и отмена подтверждение
не отправляют. Подтверждается передача потребителю потока, а не сохранение на диск.
Одновременные читатели не исключаются; потеря подтверждения может сохранить файл.

## Bounded text history

`boundedHistory:true` в capabilities обозначает поддержку
`GET /api/rooms/{room}/history?limit=30`. Ответ содержит `room` и `items` без файлов
и entry metadata. Допустимый limit — 0–30, default 30; limit=0 возвращает только
метаданные комнаты и пустой items. История идёт от новых записей к старым,
независимо от pinned. Draft/expired items скрыты. JSON элементов ограничен 8 МиБ:
возвращается помещающийся префикс; если первая запись сама превышает бюджет, ответ
413 `item_too_large`. Для отправки desktop запрашивает только метаданные комнаты.

## Upload completion retries

`idempotentUploadCompletion:true` обозначает, что повторный complete с той же
upload capability возвращает исходный 201 и метаданные файла. Результат хранится
в SQLite до истечения исходного upload TTL либо удаления файла. Повтор не меняет
manifest первого успешного complete. Это сохраняется после перезапуска сервера.
Схема SQLite 7 создаётся автоматически; protocolVersion остаётся 6, новые
возможности объявляются отдельными capability-флагами.

При передаче файла/ZIP и при приёме чанка применяется обновляемый 30-секундный
дедлайн простоя I/O вместо абсолютного ограничения всей передачи.
